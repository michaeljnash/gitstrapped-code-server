...
| `PULL_EXISTING_REPOS` |     ☐    | `true`/`false`, `t`/`f`, etc. — when repos already exist locally, `true` makes gitstrap fetch/pull/reset to `origin/<branch>`; `false` leaves them untouched.                                                                                                 |
